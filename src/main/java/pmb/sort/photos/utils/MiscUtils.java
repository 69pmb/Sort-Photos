...
public final class MiscUtils {
  ...
  public static final Predicate<TextField> isInvalidCharacters =
      f ->
  ...
  public static final BiPredicate<String, String> isValidRegex =
      (input, regex) -> Pattern.compile(regex).asMatchPredicate().test(input);

  /** Predicate to test if given input is a valid list of extension file. */
  public static final Predicate<TextField> isValidExtension =
      f -> isValidRegex.test(f.getText(), Constant.REGEX_EXTENSION);
   ...
   * @throws MajorException thrown when file or metadata can't be read (not found or corrupt for
   *     instance)
  ...
  public static Optional<String> getDimension(Metadata metadata, Object extension) {
    Map<String, Triple<Class<? extends Directory>, Integer, Integer>> infoByExtension =
        Map.of(
            "png",
            Triple.of(
                PngDirectory.class, PngDirectory.TAG_IMAGE_WIDTH, PngDirectory.TAG_IMAGE_HEIGHT),
            "jpg",
            Triple.of(
                JpegDirectory.class, JpegDirectory.TAG_IMAGE_WIDTH, JpegDirectory.TAG_IMAGE_HEIGHT),
            "webp",
            Triple.of(
                WebpDirectory.class, WebpDirectory.TAG_IMAGE_WIDTH, WebpDirectory.TAG_IMAGE_HEIGHT),
            "GIF",
            Triple.of(
                BmpHeaderDirectory.class,
                BmpHeaderDirectory.TAG_IMAGE_WIDTH,
                BmpHeaderDirectory.TAG_IMAGE_HEIGHT));
    return Optional.ofNullable(infoByExtension.get(extension))
        .flatMap(
            tuple ->
                Optional.ofNullable(metadata.getFirstDirectoryOfType(tuple.getLeft()))
                    .map(
                        d ->
                            d.getString(tuple.getMiddle())
                                + " x "
                                + d.getString(tuple.getRight())));
  }
