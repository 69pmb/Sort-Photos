...
public class DuplicateDialog extends Stage {
  ...
  public DuplicateDialog(
    ...
    LOG.info(
        "Start duplicateDialog to rename: '{}' with existing picture '{}'",
        picture.getPath(),
        existingPicture.getPath());
    ...
    gridPane.add(
        JavaFxUtils.displayPicture(
            picture.getPath(),
            Constant.CSS_CLASS_BOX,
            btnLabel,
            bundle.getString("duplicate.title.rename")),
        0,
        0);
    gridPane.add(
        JavaFxUtils.displayPicture(
            ...
            Constant.CSS_CLASS_BOX,
            btnLabel,
            bundle.getString("duplicate.title.existing")),
        1,
        0);
    ...
    hBox.getChildren()
        .add(
            new Text(
                bundle.getString(
                    picture.equals(existingPicture)
                        ? "duplicate.warning.equals"
                        : "duplicate.warning")));
    JavaFxUtils.buildButton(
        hBox,
        bundle.getString("duplicate.button.cancel"),
        e -> {
    ...
    JavaFxUtils.buildButton(
        hBox,
        bundle.getString("duplicate.button.overwrite"),
        e -> duplicateAction(() -> overwrite(picture, existingPicture)));
    JavaFxUtils.buildButton(
        hBox,
        bundle.getString("duplicate.button.rename"),
        e ->
            duplicateAction(
                () ->
                    renameWithSuffix(
                        container, bundle, exceptions, picture, existingPicture.getPath(), false)));
    JavaFxUtils.buildButton(
        hBox,
  ...
  public static void renameWithSuffix(
    ...
    String suffixedPath =
        StringUtils.substringBeforeLast(bareNewPath, Constant.SUFFIX_SEPARATOR)
            + Constant.SUFFIX_SEPARATOR
            + index
            + MyConstant.DOT
    ...
    if (!suffixedFile.exists()) {
      ...
      if (!hideDialog) {
        ...
        renameWithSuffix(
            container, bundle, exceptions, picture, suffixedFile.getAbsolutePath(), true);
      }
    }
  ...
  private static void overwrite(Picture picture, Picture existingPicture) throws MajorException {
    ...
    try {
      ...
      throw new MajorException(
          "Error moving file " + picture.getPath() + " to " + existingPicture.getPath(), e1);
    }
